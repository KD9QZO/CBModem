...
CDSP_RESAMP_TPL cdsp_rational_interpolator<SPLS_T>::cdsp_rational_interpolator(int32_t interp) {
	_interp = interp;
}

CDSP_RESAMP_TPL void cdsp_rational_interpolator<SPLS_T>::setInterpolation(int32_t interp) {
	_interp = interp;
	_interp_ctr = 0;
}

CDSP_RESAMP_TPL void cdsp_rational_interpolator<SPLS_T>::setShift(int32_t out_spls) {
	_interp_ctr = out_spls;
}

CDSP_RESAMP_TPL int cdsp_rational_interpolator<SPLS_T>::requestData(void* ctx, SPLS_T* data, int samples_cnt) {
...
CDSP_RESAMP_TPL void cdsp_rational_interpolator<SPLS_T>::_do_start() {
	_buff_avail = 0;
	_buff_ptr = 0;
}

CDSP_RESAMP_TPL void cdsp_rational_interpolator<SPLS_T>::_do_stop() {
}

CDSP_RESAMP_TPL cdsp_rational_decimator<SPLS_T>::cdsp_rational_decimator(int32_t decim) {
	setDecimation(decim);
}

CDSP_RESAMP_TPL void cdsp_rational_decimator<SPLS_T>::setDecimation(int32_t decim) {
	_decim = decim;
	_decim_ctr = 0;
}

CDSP_RESAMP_TPL int cdsp_rational_decimator<SPLS_T>::requestData(void *ctx, SPLS_T *data, int samples_cnt) {
...
CDSP_RESAMP_TPL void cdsp_rational_decimator<SPLS_T>::_do_start() {
	_buff_avail = 0;
	_buff_ptr = 0;
	_decim_ctr = 0;
}

CDSP_RESAMP_TPL void cdsp_rational_decimator<SPLS_T>::_do_stop() {
}
