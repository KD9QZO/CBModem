...
CDSP_AGC_TPL cdsp_agc<SPLS_T>::cdsp_agc(float rate) {
	setRate(rate);
}
...
CDSP_AGC_TPL int cdsp_dcblock<SPLS_T>::requestData(void *ctx, SPLS_T *data, int samples_cnt) {
	cdsp_dcblock<SPLS_T> *_this = (cdsp_dcblock<SPLS_T>*)ctx;

	if (!_this->_running || _this->_input_func==NULL) {
		return -2;
	}
...
CDSP_AGC_TPL void cdsp_dcblock<SPLS_T>::_do_start() {
	_prev_in = 0;
	_prev_out = 0;
}

CDSP_AGC_TPL void cdsp_dcblock<SPLS_T>::_do_stop() {
}
