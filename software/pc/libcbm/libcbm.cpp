...
template<typename T> ModemPILockingQueue<T>::~ModemPILockingQueue() {
	setWorking(false);
}

template<typename T> bool ModemPILockingQueue<T>::push(T const &_data) {
	if (!working) {
		return (false);
	}
...
template<typename T> bool ModemPILockingQueue<T>::empty() {
	std::lock_guard<std::mutex> lock(guard);

	return queue.empty() || !working;
}

template<typename T> size_t ModemPILockingQueue<T>::size() {
	std::lock_guard<std::mutex> lock(guard);

	return queue.size();
}

template<typename T> int ModemPILockingQueue<T>::tryPop(T &_value) {
	std::lock_guard<std::mutex> lock(guard);
...
template<typename T> bool ModemPILockingQueue<T>::waitAndPop(T &_value) {
...
template<typename T> int ModemPILockingQueue<T>::tryWaitAndPop(T &_value, int _milli) {
	std::unique_lock<std::mutex> lock(guard);
...
template<typename T> void ModemPILockingQueue<T>::setWorking(bool newwork) {
	working = newwork;
	signal.notify_one();
	signal.notify_all();
	signal.notify_all();
