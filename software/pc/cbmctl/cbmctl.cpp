...
int main(int argc, char *argv[]) {
	...
	std::string mode = (params.find("mode") == params.end()) ? "none" : params["mode"];
	char *n;
	int speed = (params.find("speed") == params.end()) ? -1 : std::strtol(params["speed"].c_str(), &n, 10);
	int centerFreq = (params.find("centerFreq") == params.end()) ? -1 : std::strtol(params["centerFreq"].c_str(), &n, 10);
	int freqDiff = (params.find("freqDiff") == params.end()) ? -1 : std::strtol(params["freqDiff"].c_str(), &n, 10);
	int frcnt = (params.find("freqCount") == params.end()) ? -1 : std::strtol(params["freqCount"].c_str(), &n, 10);
	int receive = (params.find("receive") == params.end()) ? -2 : std::strtol(params["receive"].c_str(), &n, 10);
	std::string transmit = (params.find("transmit") == params.end()) ? "" : params["transmit"];
	float fr = centerFreq;
